...
window.addEventListener("resize", () => updateProductView(currentCategory));

function refreshCartIconCount(): void {
...
function updateProductView(category: string): void {
  const container: HTMLElement | null =
    document.querySelector<HTMLElement>(".products");
...
export async function catButton(): Promise<void> {
  ...
  try {
    ...
    // showingAll = false;
    updateProductView(currentCategory);
  } catch (err) {
